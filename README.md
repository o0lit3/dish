...
For example, `dish -e 'a=[1, 2, 3]; a.map:n:i(n+a.(i+1))'` can be rewritten as `dish -e '[1, 2, 3].map:($1+$_.++$2)'`, outputting `[3, 5, 3]`, which is the sum of each item (`$1`) plus the next item (accessed by incrementing the current index `$2`), where the last item, 3, gets summed with the nonexistent 4th item (null).
