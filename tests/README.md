...
Logical operators and methods in **dish** serve as control structures for the language. **Nota bene**: Be careful with logical chains such as `(condition).then(0).else(1)`--falsy values returned by the `then` method force the `else` block into execution regardless of the condition's truthiness. Therefore, to avoid any traps of falsy values, the ternary `(condition).switch[0, 1]` is preferred.
